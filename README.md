...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Twilio (beta)](https://www.cdata.com/download/download.aspx?sku=BTZK-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Twilio supported by the [CData JDBC Driver for Twilio](https://www.cdata.com/drivers/twilio/jdbc).
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/twilio/download/jdbc](https://www.cdata.com/drivers/twilio/download/jdbc)
3. License the CData JDBC Driver:
    ...
    * Run the command `java -jar cdata.jdbc.twilio.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.twilio.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `twilio.prp`) using the following properties and format:
